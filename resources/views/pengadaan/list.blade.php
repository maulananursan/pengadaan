        ...
        <div class="row">    
          <div class="col-12">
            <div class="text-right mb-2">
              <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#pengadaanModal"><i class="fas fa-plus"></i> Tambah Data
                </button>
            </div>
            <div class="card">
                ...
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style="text-align: center;"><a target="_blank" href="{{$p->deskripsi}}"><button class="btn btn-primary btn-sm">Lihat Detail</button></a></td>
                      <td style="width: 20%; text-align: center;">
                        @if($p->gambar!= "-")
                          <img style="width: 70%;" src="{{asset(Storage::url($p->gambar))}}">
                      <hr>

                      <a href="hapusGambarPengadaan/{{$p->id_pengadaan}}" class="konfirmasi">
                        ...
                        <form method="post" action="/tambahGambarPengadaan" enctype="multipart/form-data">
                          {{csrf_field()}}
                            ...
                            <input type="file" name="gambar" id="gambar" class="form-control" style="display: none;" accept="image/png, image/jpeg, image/gif">
                      ...
                      <td>Rp. <span class="tag tag-success">{{number_format($p->anggaran,0,",",".")}}</span></td>
                      <td><a href="hapusPengadaan/{{$p->id_pengadaan}}" class="konfirmasi">
                            <button class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i> Hapus</button>
                         </a>
                         <button data-id_pengadaan="{{$p->id_pengadaan}}" data-nama_pengadaan="{{$p->nama_pengadaan}}" data-deskripsi="{{$p->deskripsi}}" data-anggaran="{{$p->anggaran}}" class="btn btn-secondary btn-sm ubah" data-toggle="modal" data-target="#ubahModal"><i class="fas fa-edit"></i> Ubah</button>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
...
<script src="{{asset('/adminLTE/plugins/toastr/toastr.min.js')}}"></script>
  ...
  $(function() {
   ...
   $(document).on("click", ".konfirmasi", function(event){
    ...
    var answer = window.confirm("Kamu yakin akan menghapus gambar ?");
